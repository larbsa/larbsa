...
algorithmCodeNameMap = {
    ...
    'larbsa': 'Larbsa algorithm, opt is for any additional config',
    'larbsa-pythag': '',
    'larbsa-pruned': '',
...
def runSingleTest (algorithm='magnetic', start=(0, 0), end=(1, 1), barriers={}, socketInformation=None, gridSize=50, comment=False, seed=None, density=None):
    ...
    elif algorithm == 'larbsa-pruned-spacing':
        tracemalloc.start()
        tracemalloc.take_snapshot()
        startTime = timer()
    ...
    elif algorithm == 'larbsa-8-pruned-spacing':
        tracemalloc.start()
        tracemalloc.take_snapshot()
        startTime = timer()
...
def runSingleTestCase (topDirectory, testCase):
    ...
    while i <= numberOfIterations:
        ...
        if isRobustness and (i % densityIncrimentAfter == 0):
            editions += 1
