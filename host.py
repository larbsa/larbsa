...
def runSingleTest (algorithm='magnetic', start=(0, 0), end=(1, 1), barriers={}, socketInformation=None, gridSize=50):
    ...
    elif algorithm == 'm4New':
        tracemalloc.start()
        tracemalloc.take_snapshot()
        startTime = timer()
        ( path, visits, message ) = magnetic4New(start, end, barriers, gridDimensions={'x':gridSize, 'y':gridSize}, maxIterations=10000000000, socketInformation=socketInformation)
        endTime = timer()
        (_, maxMemory) = tracemalloc.get_traced_memory()
        tracemalloc.stop()
